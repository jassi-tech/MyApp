...
export default function AppearanceScreen() {
  ...
  const { themeMode, setThemeMode, fontSize, setFontSize, colors, fontScale } =
    useTheme();
  ...
  const handleFontSizeChange = (size: typeof fontSize) => {
    setFontSize(size);
    showToast(
      `Font size set to ${size.replace(/([A-Z])/g, " $1").toLowerCase()}`,
    );
  };

  return (
    <ScreenContainer header={<ScreenHeader title="Appearance" />}>
      {/* Theme Selection */}
      ...
      <View style={styles.section}>
        ...
        >
          {(Object.keys(FontSizes) as Array<keyof typeof FontSizes>).map(
            (size, index, arr) => (
              <TouchableOpacity
                key={size}
              ...
              >
                <ThemedText
                  style={{ fontSize: 16 * FontSizes[size], color: colors.text }}
                >
                  {size
                    .replace(/([A-Z])/g, " $1")
      ...
      <View style={styles.section}>
        ...
        >
          ...
          <ThemedText
            style={{ color: colors.textSecondary, fontSize: 14 * fontScale }}
          >
      ...
      {toastMessage && (
        <View style={[styles.toast, { backgroundColor: colors.text }]}>
          <ThemedText style={[styles.toastText, { color: colors.background }]}>
            {toastMessage}
          </ThemedText>
        </View>
      )}
